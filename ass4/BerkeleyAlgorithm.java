...
public class BerkeleyAlgorithm {
    ...
    public static void berkeleyAlgorithm(String[] timeString) throws
ParseException {
        int n = timeString.length;
        SimpleDateFormat simpleDateFormat = new
SimpleDateFormat("HH:mm | yyyy-MM-dd");
        // Converting time to milliseconds
        long[] timeInMilliseconds = new long[n];
        for (int i = 0; i < n; i++) {
            timeInMilliseconds[i] =
simpleDateFormat.parse(timeString[i]).getTime();
}
        ...
        System.out.println("Fault tolerant average = " + avg / (1000 *60));
         // Displaying fault-tolerant average in minutes
...
continue; }
            System.out.println("Clock " + i + " adjustment = " +
offset / (1000 * 60)); // Displaying adjustment value in minutes
}
        // Converting milliseconds to actual time
        for (int i = 0; i < n; i++) {
            timeString[i] = simpleDateFormat.format(new
Date(timeInMilliseconds[i]));
} }
    private static void displayTime(String[] time, String prefix) {
        System.out.println(prefix + "Server Clock:\t" + time[0]);
        for (int i = 1; i < time.length; i++) {
            System.out.println(prefix + "Client " + i + " Clock:\t" +
time[i]); }
        System.out.println();
    }
